...
\bottomrule
\end{tabular}
\caption{\texttt{pluvialCorrectionStatus} share of the released file's 1,149,288 claims, swept over five constant precipitation thresholds. Columns correspond to \texttt{accepted\_as\_reported}, \texttt{corrected}, \texttt{no\_qualifying\_precip\_in\_window}, and, combined, \texttt{no\_aorc\_data\_in\_window} with the threshold-invariant \texttt{before\_aorc\_coverage}. Table~\ref{tab:pluvial-correction-status} gives the full $n$ claims breakdown at the default 5.0 mm threshold.}
